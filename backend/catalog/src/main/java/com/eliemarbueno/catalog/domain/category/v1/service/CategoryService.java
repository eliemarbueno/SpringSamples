...
public class CategoryService {
  ...
  public CategoryRetrieveDTO create(CategoryCreateDTO categoryDTO) {
    ...
    return new CategoryRetrieveDTO(
      categorySaved.getId(),
  ...
  public Page<CategoryRetrieveDTO> findAllCategories(
    ...
    var page = categoryRepository.findAllByNameContainingIgnoreCase(
      name,
      pageable
    );
  ...
  public void preUpdate(Category category) {
    if (
      categoryRepository
        .findByNameIgnoreCaseAndIdNot(category.getName(), category.getId())
        .isPresent()
    ) {
      throw new ItemAlreadyExistsException(
        "Category",
